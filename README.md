...
Activity Roulette is a clean, fun React Native app for generating activity ideas based on your mood and context. Choose filters like occasion, budget, indoor/outdoor preference, energy level, time of day, and weather, then spin for a polished activity card.

## Features

- Generate random activities from a local seed list of 40+ ideas.
- Filter by occasion, budget, location type, energy level, time of day, and weather.
...
The Generate button first tries to find activities that match every selected filter. If there is no exact match, the app scores each activity by how many selected filters it satisfies, then randomly chooses from the strongest relaxed matches. This keeps the result useful even when the selected combination is very specific.
